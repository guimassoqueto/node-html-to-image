import { queryByIds } from "./infra/postgres/queries/queries.js";
...
async function takeShots(jsonMessage: JsonMessage): Promise<void> {
  if (jsonMessage.k) await kadecStory(queryByIds(jsonMessage.k));

  if (jsonMessage.ts) await thuderStory(queryByIds(jsonMessage.ts));
  if (jsonMessage.tfq) await thunderFeedQuarter(queryByIds(jsonMessage.tfq));
  if (jsonMessage.tff) await thunderFeedFull(queryByIds(jsonMessage.tff));
}
