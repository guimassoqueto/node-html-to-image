...
import { POSTGRES_PRODUCTS_TABLE, POSTGRES_LAST_SCRAP_TABLE } from '../../../settings.js'
import moment from 'moment'

export class ScrapyLastExecution {
  static async getDatetime(): Promise<string> {
    const result = await pool.query(`SELECT datetime from ${POSTGRES_LAST_SCRAP_TABLE} ORDER BY datetime DESC LIMIT 1;`)
...
export class PromoThunderQueries {
  static async getProducts(datetime: string): Promise<Product[]> {
    const queryResult: QueryResult<Product> = await pool.query(`
      ...
      (
        SELECT *
        FROM ${POSTGRES_PRODUCTS_TABLE} WHERE category ILIKE ANY (ARRAY['%saúde%', '%bem-estar%', '%eletrônico%', '%tecnologia%', '%pet shop%', '%cozinha%', '%alimentos%', '%automotivo%','%bolsas%', '%casa%', '%jardim%', '%limpeza%', '%games%', '%consoles%', '%echo%', '%smartphone%', '%informática%', '%computador%', '%livro%'])
        AND reviews > 500
        AND discount >= 20
        UNION
        SELECT *
